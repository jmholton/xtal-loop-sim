...
- Handoff pass. Dead code removed (`ensure_library`, `is_current`, unused imports; their tests repointed at `library_status`), em-dashes stripped from `crystal_harvester` messages and docstrings, `drive_dcss.py --user` defaults to the current user, and the camera server's default port moved from 8080 to 8081 (GitLab's puma owns 8080 on gateway). Docs reviewed for stale facts and numbered: README 1-7 with lettered subsections, HANDOFF gained a contents list, RUNBOOK renumbered, DECISIONS gained a contents list by date. `camera_server.py`'s module docstring shrank to a pointer at the README's endpoint table. 272 tests across 21 files green, run one file at a time; DHS suite 27 green.
- The dcss database dump `sandbox/LOCAL_loopsim.txt` and `sandbox/README.md` removed; `drive_dcss.py` moved to `xtalLoopSimDHS/`. The rows dcss needs are now listed in `xtalLoopSimDHS/README.md`. DECISIONS §2026-09-23.
- xtalLoopSimDHS exercised against the local dcss rig seeded with `sandbox/LOCAL_loopsim.txt` (since removed, see above): dcss registered the five motors and three shutters, and `gtos_start_motor_move gonio_phi 90`, `gtos_start_motor_move sample_x 0.2` and `gtos_start_oscillation gonio_phi video_trigger 30 2` each completed `normal` with the camera server's pose following; the oscillation pushed 59 JPEGs to a receiver on port 9000. BluIce's phi buttons drove the spindle; its click-to-centre failed on the rig's missing `moveSample` operation. New `sandbox/drive_dcss.py` reproduces the check without BluIce.
